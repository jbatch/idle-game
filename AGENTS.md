...
Layers 1–8 (recovery + progression pass) plus the first editor-tooling, v0.2.0 progression-polish pass, v0.2.1 tech-tree layout pass, v0.2.2 playtest-onboarding pass, v0.2.3 mobile-polish pass, v0.2.4 radial tech-tree layout pass, v0.2.5 unit-sprite pass, v0.2.6 data-cache fix, v0.2.7 playtest quick-fix pass, v0.2.8 enemy-sprite pass, v0.2.9 loadout-polish pass, v0.2.10 combo/options systems pass, and v0.2.11 procedural-audio pass are complete. The game is fully playable end-to-end across all 3 chapters with a working tech tree, quest system, cheats inspector panel, combat readability effects, local scenario sandbox, capped unopened shop packs, same-unit synergy payoffs, cursor proc and combo tuning, supply pack-bonus tech, breakable battlefield crates, cursor/tower/Influence progression expansion, regenerating tower shields, support-unit crate opening, first-time onboarding spotlights, scalable canvas/mobile shims, local GUI editors for tech tree, unit data, and audio selection, a programmatic radial tech-tree layout prototype, static SVG paper-doll sprites for every player unit and enemy/boss, versioned JSON/SVG/audio config asset requests, playtest UX fixes, polished SVG loadout previews, persisted audio options, and procedural SFX/music. This is the **v0.2.11 checkpoint**.
...
- Unit visual metadata is validated by `pnpm validate:data`, and the main menu version label now reads `v0.2.5 unit sprites`.
...
- Main menu version label now reads `v0.2.6 unit sprites`.
...
- Main menu version label now reads `v0.2.7 playtest quick fixes`.
...
- Main menu version label now reads `v0.2.8 enemy sprites`.
...
### v0.2.11 Additions
- The game now uses a procedural Web Audio engine for SFX and background music instead of empty Phaser audio placeholders.
- Local Audio Lab exists at `/tools/audio-lab.html` with three selectable variants for each SFX and music hook.
- Audio selections are saved to `public/data/audio_config.json`, validated by `pnpm validate:data`, loaded by the game at boot, and versioned with the data cache key.
- Cursor hits and shield absorbs now have live SFX hooks alongside the existing UI, pack, crate, combo, boss, victory, and defeat hooks.
- Game data cache version now reads `0.2.11`, and the main menu version label now reads `v0.2.11 procedural audio`.
