...
- Gated the shop Tech Tree entry until after the first recorded run, so fresh players are pushed into the first combat/tutorial flow before seeing progression. Updated the first tech-tree tip to call out that +DC upgrades are very valuable as soon as they appear.
